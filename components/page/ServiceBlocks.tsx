      ...
      {blocks.map((block: Block, idx: number) => {
        ...
        switch (type) {
          case "blocks.hero": {
            ...
            const subtitle = pickFirstString(block, [
              "Subtitle",
              "subtitle",
              "description",
            ]);
            ...
            return (
              <section
                key={idx}
                className="py-20 bg-gradient-to-br from-red-50 via-white to-pink-50"
              >
          ...
          case "blocks.heading-section": {
            ...
            const description = pickFirstString(block, [
              "description",
              "Subtitle",
              "text",
            ]);
                  ...
                  <div className="relative">
                    ...
                    <div className="space-y-12">
                      {steps.map((step: any, i: number) => {
                        const stepNumber =
                          pickFirstString(step, ["stepNumber", "step"]) ||
                          String(i + 1).padStart(2, "0");
                        const stepTitle =
                          pickFirstString(step, ["title", "heading"]) || "Step";
                        const stepDescription = pickFirstString(step, [
                          "description",
                          "text",
                        ]);
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
                  ...
                  <div className="grid md:grid-cols-3 gap-6">
                    {cards.map((card: any, cardIdx: number) => {
                      ...
                      const cardTitle = pickFirstString(card, [
                        "title",
                        "Heading",
                      ]);
                      const cardDescription = pickFirstString(card, [
                        "description",
                        "text",
                      ]);
                      const cardLinkHref = resolveLinkHref(card?.link);
                      const cardLinkLabel = resolveLinkLabel(card?.link);
                      const lists = Array.isArray(card?.lists)
                        ? card.lists
                        : [];
          ...
          case "blocks.services": {
            ...
            const linkLabel = resolveLinkLabel(block?.link);
            const gradient =
              pickFirstString(block, ["color"]) || "from-red-500 to-red-600";
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  >
                    <div className="flex-1 space-y-6 text-left">
                      {iconName && (
                        <div
                          className={`w-16 h-16 bg-gradient-to-r ${gradient} rounded-2xl flex items-center justify-center text-white`}
                        >
                          <LucideIcon
                            name={iconName}
                            className="w-8 h-8 text-white"
                      ...
                      {items.length > 0 && (
                        <ul className="space-y-3">{renderListItems(items)}</ul>
                      )}
                      {linkHref && (
                        <Link href={linkHref}>
                          <Button className="bg-gradient-to-r from-red-500 to-red-600 hover:opacity-90">
                    ...
                    <div className="flex-1 w-full">
                      <div
                        className={`w-full min-h-[220px] rounded-3xl bg-gradient-to-br ${gradient} flex items-center justify-center p-8`}
                      >
          ...
          case "blocks.comparison-grid": {
            ...
            const columnLabels = columns.map(
              (c: any) =>
                pickFirstString(c, ["listItem", "title", "text"]) || ""
            );
                  ...
                  <div className="overflow-x-auto">
                    <div className="min-w-full">
                      <div className="grid grid-cols-4 gap-4 mb-4 text-sm font-semibold text-gray-700">
                        {columnLabels.slice(0, 4).map((label, i) => (
                          <div key={i}>{label}</div>
                        ))}
                      </div>
                      <div className="space-y-4">
                        {rows.map((row: any, rIdx: number) => {
                          const color =
                            pickFirstString(row, ["color"]) ||
                            "from-red-500 to-red-600";
                          const iconName =
                            pickFirstString(row, ["icon"]) || "Briefcase";
                          const permitType =
                            pickFirstString(row, ["permitType"]) || "";
                          const lmiaRequired =
                            pickFirstString(row, ["lmiaRequired"]) || "";
                          const bestFor =
                            pickFirstString(row, ["bestFor"]) || "";
                          const duration =
                            pickFirstString(row, ["duration"]) || "";
                          return (
                            <Card
                              key={rIdx}
                            ...
                            >
                              <CardContent className="p-4">
                                <div className="grid grid-cols-4 gap-4 items-center">
                                  ...
                                  <div className="text-gray-600">{bestFor}</div>
                                  <div className="text-gray-600">
                                    {duration}
                                  </div>
                                </div>
